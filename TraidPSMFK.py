...
class TraidPSMFK():
    ...
    def __get_M(self):
        return np.array([[0, 1, 0, 0], [self.theta_sin, 0, -self.theta_cos, self.l1-self.l2-self.l3],
                         [-self.theta_cos, 0, -self.theta_sin, self.l4], [0, 0, 0, 1]])
    ...
    def __twist2ht(self, S, theta):
        ...
        if np.linalg.norm(omega) < 1e-8:
            ...
            p = np.dot((np.eye(3)*theta + (1-np.cos(theta))*omega_skew + (theta-np.sin(theta))*np.dot(omega_skew,
                                                                                                      omega_skew)), v)
        T[0:3, 0:3] = R
        T[0:3, 3] = p
        return T
